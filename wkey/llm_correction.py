...
def _resolve_llm_runner():
    ...
    if provider == "groq":
        model = custom_model or "openai/gpt-oss-20b"
        return (
            "groq",
            lambda messages: groq_provider.run_chat_completion(messages, model=model),
        )
    ...
    return (
        "openai",
        lambda messages: openai_provider.run_chat_completion(messages, model=model),
    )
...
def create_llm_corrector():
    ...
    def corrector(transcript: str) -> str:
        ...
        provider_name, runner = _resolve_llm_runner()
        try:
            print("Correcting...")
            print(f"Calling LLM provider: {provider_name}")
            ...
            sanitized_transcript = _strip_think_blocks(corrected_transcript)
            print("LLM response without thinking:")
            print(sanitized_transcript)

            # Add the corrected transcript to the history for the next turn's context.
            history.append(sanitized_transcript)
            return sanitized_transcript

        except Exception as e:
            print(f"Error calling LLM API {provider_name}: {e}")
